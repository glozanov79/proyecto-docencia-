"""
Mini-agente: Teoría Comercio Internacional
...
def es_receso_o_festivo(nota):
    if not nota:
        return False
    keywords = ("receso", "festivo", "no hay clase", "cancelada", "suspendida", "avance")
    return any(k in nota.lower() for k in keywords)


# ── Parseo del temario ─────────────────────────────────────────────────────

def tema_de_semana(semana, calendario):
    """Extrae tema y recurso del calendario."""
    for t in calendario.get("temario", []):
        if t["semana"] == semana:
            return t["tema"], t["recurso"]
    return "—", "—"
...
def objetivos_desde_tema(tema):
    ...
    objetivos.append("- Relacionar el tema con el análisis de mercados financieros internacionales")
    return "\n".join(objetivos)
...
def generar_brief(fecha_iso, sesion, curso, continuidad_md, calendario):
    semana = sesion["semana"]
    tema, recurso = tema_de_semana(semana, calendario)
    objetivos = objetivos_desde_tema(tema)
    contenido = contenido_clave_desde_tema(tema)

    tema_siguiente, _ = tema_de_semana(semana + 1, calendario)
    proximo_tema = tema_siguiente if tema_siguiente != "—" else "Cierre del curso"
...
def main():
    ...
    brief = generar_brief(fecha_objetivo, sesion, curso, continuidad_md, calendario)
    ...
    tema, _ = tema_de_semana(sesion["semana"], calendario)
